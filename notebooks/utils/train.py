...
def train_model(training_set):
    ...
    if num_rows < 2:
        raise ValueError("Argument training_set must have at least 2 rows for linear regression to work, it actually has just {0}".format(num_rows))
    num_cols = training_set.shape[1]
    if num_cols != 2:
        raise ValueError("Argument training_set must have 2 columns for univariate linear regression. It actually has {0} columns".format(num_cols))
